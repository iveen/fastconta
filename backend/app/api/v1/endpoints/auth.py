...
async def login(login_data: LoginRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if must_change:
        logger.info(
            f"⚠️ Usuario {user.email} debe cambiar contraseña "
    ...
    await log_login_attempt(db, user.id, login_data.email, ip_address, user_agent, "SUCCESS", None)
...
async def request_password_reset(
    ...
    if not user:
        ...
        return RequestPasswordResetResponse(message="Si el email existe, recibirás un correo con instrucciones.")
...
async def get_login_audit(
    ...
    status: str | None = Query(
        None,
        description="Filtrar por status: SUCCESS, FAILED_INVALID_PASSWORD, FAILED_LOCKED, FAILED_USER_NOT_FOUND, FAILED_INACTIVE",
    ),
...
async def get_login_audit_stats(
    ...
    # Totales por status
    total = await db.scalar(select(func.count(LoginAudit.id)).where(LoginAudit.created_at >= since)) or 0

    successful = (
        await db.scalar(
            select(func.count(LoginAudit.id)).where(LoginAudit.created_at >= since, LoginAudit.status == "SUCCESS")
        )
        or 0
    )
    ...
    locked = (
        await db.scalar(
            select(func.count(LoginAudit.id)).where(
                LoginAudit.created_at >= since, LoginAudit.status == "FAILED_LOCKED"
            )
        )
        or 0
    )
    ...
    unique_users = (
        await db.scalar(
            select(func.count(func.distinct(LoginAudit.user_id))).where(
                LoginAudit.created_at >= since, LoginAudit.user_id.isnot(None)
            )
        )
        or 0
    )
    ...
    unique_ips = (
        await db.scalar(
            select(func.count(func.distinct(LoginAudit.ip_address))).where(
                LoginAudit.created_at >= since, LoginAudit.ip_address.isnot(None)
            )
        )
        or 0
    )
    ...
    top_failed_query = await db.execute(
        select(LoginAudit.email_attempted, func.count(LoginAudit.id).label("count"))
        .where(LoginAudit.created_at >= since, LoginAudit.status != "SUCCESS")
        .group_by(LoginAudit.email_attempted)
        .order_by(func.count(LoginAudit.id).desc())
        .limit(5)
    )
    most_failed_emails = [{"email": row.email_attempted, "count": row.count} for row in top_failed_query.all()]
