...
async def public_register(payload: TenantRequestCreate, request: Request, db: AsyncSession = Depends(get_db)):
    """
    ...
    if attempts_count >= 3:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Demasiados registros desde tu IP. Intenta más tarde."
        )

    # 2. Validar que no exista un tenant activo con ese NIT
    existing_tenant = await db.execute(select(Tenant).where(Tenant.nit == payload.nit, Tenant.is_active.is_(True)))

    if existing_tenant.scalar_one_or_none():
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Ya existe una cuenta activa con este NIT.")
    ...
    if existing_request.scalar_one_or_none():
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Ya existe una solicitud activa con este NIT o email."
        )
