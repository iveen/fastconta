...
async def _resolve_empresa_by_public_id(db: AsyncSession, schema_name: str, public_id: UUID) -> int:
    """Helper para resolver una empresa por su public_id y retornar el id (BIGINT)."""
    await db.execute(text(f"SET LOCAL search_path TO {schema_name}, public"))
    result = await db.execute(text("SELECT id FROM empresas WHERE public_id = :pid LIMIT 1"), {"pid": str(public_id)})
    row = result.first()
    if not row:
...
async def get_user_empresas(
    ...
    res_assignments = await db.execute(
        select(UserEmpresa.empresa_id).where(UserEmpresa.user_id == user.id, UserEmpresa.is_active.is_(True))
    )
    ...
    res_tenant = await db.execute(select(Tenant.schema_name).where(Tenant.id == user.tenant_id))
    ...
    res_empresas = await db.execute(
        text(f"SELECT public_id, nombre, nit FROM empresas WHERE id IN ({placeholders})"), params
    )
...
@router.post(
    ...
    dependencies=[Depends(require_role("tenant_manager", "superadmin")), Depends(require_write_access)],
)
async def assign_empresa_to_user(
    ...
    res_tenant = await db.execute(select(Tenant.schema_name).where(Tenant.id == tenant_id))
    schema_name = res_tenant.scalar_one_or_none()
    ...
    empresa_id_bigint = await _resolve_empresa_by_public_id(db, schema_name, payload.empresa_id)
    ...
    assignment = UserEmpresa(user_id=target_user.id, tenant_id=tenant_id, empresa_id=empresa_id_bigint, is_active=True)
    db.add(assignment)
    await db.commit()
