...
from app.schemas.catalogos.categoria_activo import (
    ...
    CategoriaActivoListResponse,
    CategoriaActivoResponse,
...
async def listar_categorias(
    ...
    return {
        "data": [CategoriaActivoListResponse.model_validate(c) for c in categorias],
        "total": total,
        "skip": skip,
        "limit": limit,
    }
...
@router.get("/activos", response_model=list[CategoriaActivoListResponse])
...
@router.get("/{categoria_id}", response_model=CategoriaActivoResponse)
...
async def crear_categoria(
    data: CategoriaActivoCreate, service: CategoriaActivoService = Depends(get_service)
):
    try:
...
@router.patch("/{categoria_id}", response_model=CategoriaActivoResponse)
async def actualizar_categoria(
    categoria_id: UUID,
    data: CategoriaActivoUpdate,
    service: CategoriaActivoService = Depends(get_service),
):
