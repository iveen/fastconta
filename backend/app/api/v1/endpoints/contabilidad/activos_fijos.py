...
from app.models.tenant_models import ActivoFijo, Empresa
...
from app.services.contabilidad import activos_fijos_service
...
async def listar_categorias_activos(db: AsyncSession = Depends(get_public_db)):
...
async def obtener_siguiente_codigo(
    empresa_id: int | None = Query(None, description="ID de la empresa (opcional, usa X-Company-Id)"),  # ✅ BIGINT
    categoria_id: int = Query(..., description="ID de la categoría"),  # ✅ BIGINT
    tenant_id: int | None = Query(None, description="ID del tenant (para superadmin)"),  # ✅ BIGINT
    ...
    stmt_max = select(func.max(func.cast(func.split_part(ActivoFijo.codigo_interno, "-", 2), Integer))).where(
        ActivoFijo.empresa_id == empresa_id_final, ActivoFijo.codigo_interno.like(f"{prefijo}-%")
    )
...
async def procesar_cierre_mensual(
    ...
    return await activos_fijos_service.procesar_depreciacion_mensual_async(
        db=db, empresa_id=request.empresa_id, anio=request.anio_periodo, mes=request.mes_periodo
    )
...
async def listar_activos(
    empresa_id: int = Depends(verificar_acceso_empresa),  # ✅ BIGINT
    skip: int = 0,
    limit: int = 100,
    db: AsyncSession = Depends(get_tenant_db),
):
...
async def obtener_activo(
    ...
    stmt = (
        ...
        .where(ActivoFijo.id == activo_id, ActivoFijo.empresa_id == empresa_id)
    )
    result = await db.execute(stmt)
...
async def crear_activo(
    ...
    return await activos_fijos_service.crear_activo_fijo_async(db=db, empresa_id=empresa_id, data=data)
...
async def actualizar_activo(
    ...
    stmt = select(ActivoFijo).where(ActivoFijo.id == activo_id, ActivoFijo.empresa_id == empresa_id)
    result = await db.execute(stmt)
    ...
    return await activos_fijos_service.actualizar_activo_fijo_async(db=db, activo_id=activo_id, data=data)
...
async def obtener_proyeccion_depreciacion(
    ...
    return await activos_fijos_service.obtener_proyeccion_depreciacion_async(db=db, activo_id=activo_id)
