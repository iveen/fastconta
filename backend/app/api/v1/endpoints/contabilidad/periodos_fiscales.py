...
async def crear_periodo(payload: PeriodoFiscalCreate, db: AsyncSession = Depends(get_tenant_db)):
    ...
    existente = await db.execute(
        select(PeriodoFiscal).where(
            PeriodoFiscal.nombre == payload.nombre, PeriodoFiscal.empresa_id == payload.empresa_id
        )
    )
