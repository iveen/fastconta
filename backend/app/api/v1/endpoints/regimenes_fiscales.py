...
from app.schemas.configuracion_fiscal.regimen_fiscal import (
    RegimenFiscalCreate,
...
from app.services.configuracion_fiscal.regimen_fiscal_service import (
...
async def listar_regimenes(
    ...
    return {
        "data": [RegimenFiscalListResponse.model_validate(r) for r in regimenes],
        "total": total,
        "skip": skip,
        "limit": limit,
    }
...
# CREAR
# ============================================================
@router.post("/", response_model=RegimenFiscalResponse, status_code=status.HTTP_201_CREATED)
async def crear_regimen(
    ...
    try:
        regimen = await service.crear(data.model_dump())
        return regimen
