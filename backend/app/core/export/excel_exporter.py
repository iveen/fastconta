...
class ExcelExporter:
    ...
    def export(report: ReportDefinition) -> io.BytesIO:
        ...
        header_fill = PatternFill(
            start_color=report.header_background, end_color=report.header_background, fill_type="solid"
        )
        thin_border = Border(
        ...
        # ===== HEADER DEL REPORTE =====
        ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, end_column=len(report.columns))
        ...
        for section in report.sections:
            ...
            if section.title:
                ws.merge_cells(
                    start_row=current_row, start_column=1, end_row=current_row, end_column=len(report.columns)
                )
            ...
            for col_idx, col in enumerate(report.columns, 1):
                ...
                cell.alignment = Alignment(horizontal=col.alignment.value, vertical="center")
            ...
            for row_idx, row in enumerate(section.rows):
                for col_idx, col in enumerate(report.columns, 1):
                    ...
                    if row.background_color:
                        cell.fill = PatternFill(
                            start_color=row.background_color, end_color=row.background_color, fill_type="solid"
                        )
                ...
                if row.indent_level > 0:
                    ...
                    first_cell.alignment = Alignment(horizontal="left", indent=row.indent_level * 2)
        ...
        if report.generated_at:
            ...
            ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, end_column=len(report.columns))
            cell = ws.cell(
                row=current_row, column=1, value=f"Generado el {report.generated_at.strftime('%d/%m/%Y %H:%M')}"
            )
