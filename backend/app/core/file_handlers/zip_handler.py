...
class ZipFileHandler(FileHandler):
    ...
    async def read(self, upload_file: UploadFile) -> FileContent:
            ...
            with zipfile.ZipFile(io.BytesIO(raw), "r") as zip_ref:
                for file_info in zip_ref.infolist():
                    if file_info.filename.lower().endswith(".xml"):
                        ...
                        xml_files.append({"filename": file_info.filename, "xml_text": xml_text, "raw_bytes": xml_bytes})
        ...
        return FileContent(
            ...
            extension="zip",
            parsed_data={"xml_files": xml_files, "total_files": len(xml_files), "source": "zip"},
        )
