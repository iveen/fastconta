...
async def get_data_scope(
    ...
    ROLE_LEVELS = {
        "superadmin": (100, False),
        "admin": (80, False),
        "tenant_manager": (80, False),
        "tenant_member": (60, False),
        "contador": (60, False),
        "auxiliar": (40, False),
        "tenant_client": (20, True),
        "cliente": (20, True),
    }
    nivel_acceso, is_read_only = ROLE_LEVELS.get(role_code, (0, True))

    scope = DataScope(user=current_user, role_code=role_code, nivel_acceso=nivel_acceso, is_read_only=is_read_only)
...
def require_role(*allowed_roles: str):
    def checker(scope: DataScope = Depends(get_data_scope)):
        if scope.role_code not in [r.lower() for r in allowed_roles]:
            raise HTTPException(
                status.HTTP_403_FORBIDDEN, f"Acceso denegado. Roles permitidos: {', '.join(allowed_roles)}"
            )
        return scope
...
def is_password_expired(user) -> bool:
    ...
    if not user.password_expires_at:
        return False
    return datetime.now(UTC) > user.password_expires_at
...
def is_lock_expired(user) -> bool:
    ...
    """
    return user.is_locked and user.locked_until is not None and datetime.now(UTC) > user.locked_until
