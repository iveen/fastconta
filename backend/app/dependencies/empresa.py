...
async def get_active_empresa(
...
) -> Empresa | None:  # ✅ Corregido: permite retornar None si no hay header
    """
    ...
    # 1. Validar formato (ahora es BIGINT/entero, no UUID)
    try:
        empresa_id = int(x_company_id)  # ✅ CORREGIDO: int() en lugar de UUID()
    except ValueError:
        raise HTTPException(
