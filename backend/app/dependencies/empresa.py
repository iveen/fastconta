...
async def get_active_empresa(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="El identificador de empresa (X-Company-Id) no es válido."
        )
    ...
    if not empresa:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Empresa no encontrada. Verifique que tenga acceso a ella."
        )
