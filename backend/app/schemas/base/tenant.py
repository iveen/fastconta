...
class TenantTrialRequest(BaseModel):
    ...
    trial_days: int = Field(..., gt=0, le=365, description="Días de duración del trial (1-365)")
    trial_max_usuarios: int = Field(..., gt=0, le=1000, description="Límite de usuarios durante el trial")
...
class TenantUpgradeRequest(BaseModel):
    ...
    max_usuarios: int = Field(..., gt=0, description="Nuevo límite permanente de usuarios")
    plan: str = Field(
        default="basic", pattern="^(freemium|basic|pro|enterprise)$", description="Plan comercial del tenant"
    )
