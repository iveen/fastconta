...
async def seed_municipios(db, dept_map: dict):
    ...
    for mun_data in MUNICIPIOS:
        ...
        if not departamento_id:
            print(
                f"  ⚠️  Error: No se encontró departamento con código {dept_codigo} para municipio {mun_data['nombre']}"
            )
            continue
        ...
        if existente:
            ...
            # Crear nuevo
            nuevo = Municipio(
                codigo_iso=mun_data["codigo_iso"], nombre=mun_data["nombre"], departamento_id=departamento_id
            )
            db.add(nuevo)
