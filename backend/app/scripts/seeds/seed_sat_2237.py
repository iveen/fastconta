        ...
        try:
            ...
            for cas_data in CASILLAS_SAT_2237:
                ...
                stmt = select(CasillaSat).where(
                    CasillaSat.seccion_id == seccion_id, CasillaSat.codigo == cas_data["codigo"]
                )
                result = await db.execute(stmt)
            ...
            stmt_casillas = select(CasillaSat.codigo, CasillaSat.id).where(
                CasillaSat.seccion_id.in_(list(secciones_map.values()))
            )
            ...
            for regla_data in REGLAS_FILTRADO_SAT_2237:
                ...
                stmt = select(ReglaFiltradoFactura).where(
                    ReglaFiltradoFactura.casilla_id == casilla_id, ReglaFiltradoFactura.nombre == regla_data["nombre"]
                )
                result = await db.execute(stmt)
            ...
            for exc_data in EXCLUSIONES_SAT_2237:
                ...
                stmt = select(ExclusionCasilla).where(
                    ExclusionCasilla.casilla_id == casilla_id, ExclusionCasilla.nombre == exc_data["nombre"]
                )
                result = await db.execute(stmt)
            ...
            for reg_codigo in REGIMENES_ASOCIADOS:
                ...
                stmt = select(RegimenFormularioSat).where(
                    RegimenFormularioSat.regimen_id == regimen_id, RegimenFormularioSat.formulario_id == formulario.id
                )
                result = await db.execute(stmt)
                ...
                if not asociacion:
                    nueva_asociacion = RegimenFormularioSat(
                        regimen_id=regimen_id, formulario_id=formulario.id, es_obligatorio=True, created_by=None
                    )
