...
def _dias_en_mes_adquisicion(fecha_adquisicion: date, anio: int, mes: int) -> int:
    ...
    Política inclusiva: incluye el día de compra.
    """
    _, ultimo_dia = calendar.monthrange(anio, mes)
    # Solo aplica si el mes/año coincide con la fecha de adquisición
...
def _dias_en_mes_final(fecha_adquisicion: date, vida_util_meses: int, anio: int, mes: int) -> int | None:
    ...
    Retorna None si el mes no es el último mes.
    """
    # Fecha exacta en que termina la vida útil
    # Sumamos vida_util_meses a la fecha de adquisición
    ...
    # Si el mes actual es el mes final de vida útil
    if anio == anio_fin and mes == mes_fin:
        return dia_fin - 1  # Días transcurridos en ese mes (excluye el día final)
...
async def _calcular_depreciacion_mes(
    ...
    Incluye prorrateo por días para el primer y último mes.
    """
    # 1. Verificar si ya existe registro para este periodo (idempotencia)
    ...
    # 6. DETERMINAR SI ES PRIMER MES O ÚLTIMO MES (PRORRATEO)
    es_primer_mes = (
        activo.fecha_adquisicion.year == anio and 
        activo.fecha_adquisicion.month == mes
    )
    
    dias_mes_final = _dias_en_mes_final(
        activo.fecha_adquisicion, 
        activo.vida_util_meses_aplicada, 
        anio, 
        mes
    )
    es_ultimo_mes = dias_mes_final is not None

    # 7. Calcular monto a depreciar según el caso
    if es_primer_mes:
        # PRIMER MES: Prorrateo por días de uso
        ...
        # ÚLTIMO MES: Solo lo que falta para llegar al valor residual
        monto_a_depreciar = _redondear(base_depreciable - depreciacion_acumulada_anterior)
    else:
        # MES NORMAL: Depreciación completa estándar
        monto_a_depreciar = depreciacion_mensual_estandar

    # 8. Validar que no exceda la base depreciable
    ...
    # 9. Calcular nuevos acumulados
    nueva_depreciacion_acumulada = _redondear(
        depreciacion_acumulada_anterior + monto_a_depreciar
    )
...
# ==============================================================================
# SERVICIOS PRINCIPALES (ASYNC)
# ==============================================================================
async def crear_activo_fijo_async(db: AsyncSession, empresa_id: uuid.UUID, data: ActivoFijoCreate) -> ActivoFijo:
    ...
    # 3. Validar que se proporcionen cuentas contables
    ...
    # await db.refresh(nuevo_activo)
    await db.commit()
    
    return activo_con_relaciones

async def actualizar_activo_fijo_async(db: AsyncSession, activo_id: uuid.UUID, data: ActivoFijoUpdate) -> ActivoFijo:
...
async def procesar_depreciacion_mensual_async(
    ...
    Genera una única Partida de Diario consolidada.
    """
    ...
    for activo in activos:
        ...
        if calculo and calculo["monto_depreciacion_mes"] > 0:
            ...
            # Crear registro histórico de depreciación
    ...
    await db.flush() # Obtener el ID de la partida recién creada

    # 6. Crear los detalles de la partida
    ...
    # 7. Vincular los registros de depreciación con la partida
...
async def obtener_proyeccion_depreciacion_async(db: AsyncSession, activo_id: uuid.UUID) -> TablaDepreciacionProyectadaResponse:
    """
    Genera una proyección futura de la depreciación para mostrar en el frontend.
    Incluye la bandera 'es_historico' para diferenciar visualmente periodos pasados de futuros.
    """
    stmt = select(ActivoFijo).where(ActivoFijo.id == activo_id)
    result = await db.execute(stmt)
    activo = result.scalar_one_or_none()
    ...
    # Convertir a diccionario para búsqueda rápida O(1)
    ...
    while acumulado_proyectado < base_depreciable and iteraciones < max_iteraciones:
        ...
        if (anio_actual, mes_actual) in historial_dict:
            ...
            # Determinar si es primer mes o último mes
            es_primer_mes = (
                activo.fecha_adquisicion.year == anio_actual and 
                activo.fecha_adquisicion.month == mes_actual
            )
            dias_mes_final = _dias_en_mes_final(
                activo.fecha_adquisicion,
                activo.vida_util_meses_aplicada,
            ...
            # Validar no exceder base depreciable
        ...
        proyeccion.append(TablaDepreciacionProyectadaItem(
            ...
            nota=nota if 'nota' in locals() else None
        ))
