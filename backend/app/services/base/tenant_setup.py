...
def validate_schema_name(schema_name: str) -> None:
    ...
    if schema_name.lower() in RESERVED_SCHEMAS:
        raise ValueError(f"El schema '{schema_name}' está reservado y no puede usarse como tenant")
    ...
    if len(schema_name) > 63:
        raise ValueError(f"Schema inválido: '{schema_name}' excede el límite de 63 caracteres")
...
def _create_schema_with_privileges(schema_name: str) -> None:
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.execute(text(f'ALTER DEFAULT PRIVILEGES IN SCHEMA "{schema_name}" GRANT ALL ON TABLES TO "{db_user}"'))
            conn.execute(
                text(f'ALTER DEFAULT PRIVILEGES IN SCHEMA "{schema_name}" GRANT ALL ON SEQUENCES TO "{db_user}"')
            )
...
def cleanup_tenant_schema(schema_name: str) -> None:
    ...
    if schema_name.lower() in RESERVED_SCHEMAS:
        logger.warning(f"⚠️ Intento de eliminar schema reservado '{schema_name}'. Operación bloqueada.")
        return
