...
class GeografiaService:
    ...
    async def obtener_departamentos(self) -> list[Departamento]:
        query = select(Departamento).options(selectinload(Departamento.municipios)).order_by(Departamento.codigo_iso)
        result = await self.db.execute(query)
        return list(result.scalars().all())

    async def obtener_departamento_por_id(self, depto_id: UUID) -> Departamento | None:
        query = select(Departamento).options(selectinload(Departamento.municipios)).where(Departamento.id == depto_id)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def crear_departamento(self, data: dict) -> Departamento:
        existente = await self.db.execute(select(Departamento).where(Departamento.codigo_iso == data["codigo_iso"]))
    ...
    async def obtener_municipio_por_id(self, mun_id: UUID) -> Municipio | None:
        query = select(Municipio).options(selectinload(Municipio.departamento)).where(Municipio.id == mun_id)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def crear_municipio(self, data: dict) -> Municipio:
        existente = await self.db.execute(select(Municipio).where(Municipio.codigo_iso == data["codigo_iso"]))
        if existente.scalar_one_or_none():
