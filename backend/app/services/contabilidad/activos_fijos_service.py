...
async def _calcular_depreciacion_mes(activo: ActivoFijo, anio: int, mes: int, db: AsyncSession) -> dict | None:
    ...
    stmt = select(func.coalesce(func.sum(DepreciacionActivo.monto_depreciacion_mes), 0)).where(
        ...
        or_(
            DepreciacionActivo.anio_periodo < anio,
            and_(DepreciacionActivo.anio_periodo == anio, DepreciacionActivo.mes_periodo < mes),
        ),
    )
    result = await db.execute(stmt)
...
async def crear_activo_fijo_async(db: AsyncSession, empresa_id: int, data: ActivoFijoCreate) -> ActivoFijo:  # ✅ BIGINT
    ...
    stmt = select(CategoriaActivoFijo).where(
        CategoriaActivoFijo.id == data.categoria_id, CategoriaActivoFijo.is_active.is_(True)
    )
    result = await db.execute(stmt)
...
async def actualizar_activo_fijo_async(
    db: AsyncSession, activo_id: int, data: ActivoFijoUpdate
) -> ActivoFijo:  # ✅ BIGINT
...
async def procesar_depreciacion_mensual_async(
    ...
    stmt = select(ActivoFijo).where(ActivoFijo.empresa_id == empresa_id, ActivoFijo.estado == "activo")
    result = await db.execute(stmt)
    ...
    for cuenta_id, monto in movimientos_debe.items():
        db.add(DetallePartida(partida_id=nueva_partida.id, cuenta_id=cuenta_id, tipo_movimiento="debe", monto=monto))

    for cuenta_id, monto in movimientos_haber.items():
        db.add(DetallePartida(partida_id=nueva_partida.id, cuenta_id=cuenta_id, tipo_movimiento="haber", monto=monto))
...
async def obtener_proyeccion_depreciacion_async(
    db: AsyncSession, activo_id: int
) -> TablaDepreciacionProyectadaResponse:  # ✅ BIGINT
    ...
    stmt = (
        select(DepreciacionActivo)
        .where(DepreciacionActivo.activo_id == activo_id)
    ...
    while acumulado_proyectado < base_depreciable and iteraciones < max_iteraciones:
        ...
        if (anio_actual, mes_actual) in historial_dict:
            ...
            es_primer_mes = (
                activo.fecha_adquisicion.year == anio_actual and activo.fecha_adquisicion.month == mes_actual
            )
            dias_mes_final = _dias_en_mes_final(
                activo.fecha_adquisicion, activo.vida_util_meses_aplicada, anio_actual, mes_actual
            )
            es_ultimo_mes = dias_mes_final is not None
