...
async def ejecutar_cierre_anual(
    ...
    try:
        ...
        if not periodo or periodo.empresa_id != empresa_id:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Período fiscal no encontrado o no pertenece a la empresa"
            )
        if periodo.cerrado:
        ...
        # =========================================================================
        patrones_cierre = [f"CIE-{periodo.nombre}", f"AJU-{periodo.nombre}", f"REV-{periodo.nombre}"]
        stmt_buscar_previas = select(Partida).where(
            Partida.empresa_id == empresa_id, Partida.numero_poliza.in_(patrones_cierre)
        )
        ...
        stmt_saldo_anterior = (
            ...
            .where(DetallePartida.cuenta_id == empresa.cuenta_utilidad_periodo_id, Partida.fecha < periodo.fecha_inicio)
        )
            ...
            if cuenta.tipo == "ingreso":
                ...
                if net_balance != 0:
                    ...
                    detalles_cierre.append(
                        {"cuenta_id": cuenta.id, "tipo_movimiento": "debe", "monto": abs(net_balance)}
                    )
                ...
                if net_balance != 0:
                    ...
                    detalles_cierre.append(
                        {"cuenta_id": cuenta.id, "tipo_movimiento": "haber", "monto": abs(net_balance)}
                    )
