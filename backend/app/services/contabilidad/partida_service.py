...
class PartidaService:
    ...
    async def validar_cuentas_unicas(self, cuenta_ids: list[int]) -> None:
        ...
        for cuenta_id in cuenta_ids:
            ...
            result = await self.db.execute(select(CuentaContable.codigo).where(CuentaContable.id == cuenta_id))
    ...
    async def validar_poliza_unica(
        self, numero_poliza: str, empresa_id: int, partida_id_excluir: int | None = None
    ) -> None:
        """Valida que el número de póliza no exista"""
        stmt = select(Partida).where(Partida.numero_poliza == numero_poliza, Partida.empresa_id == empresa_id)
    ...
    async def modificar_partida(
        self, partida_id: int, fecha: date, descripcion: str, detalles: list[dict], numero_poliza: str | None = None
    ) -> PartidaOut:
        ...
        await self.db.execute(delete(DetallePartida).where(DetallePartida.partida_id == partida_id))
    ...
    async def revertir_partida(
        self, partida_id: int, fecha_reversion: date | None = None, schema_name: str | None = None
    ) -> PartidaOut:
        ...
        await self.db.execute(stmt_update, {"partida_reversion_id": partida_reversion_id, "partida_id": partida_id})
    ...
    async def listar_partidas(self, empresa_id: int | None = None) -> list[Partida]:
        ...
        stmt = (
            select(Partida)
            .options(
                selectinload(Partida.detalles).selectinload(DetallePartida.cuenta).selectinload(CuentaContable.empresa),
    ...
    async def obtener_libro_diario(self, empresa_id: int, fecha_inicio: date, fecha_fin: date) -> list[dict]:
        ...
        stmt = (
            ...
            .where(CuentaContable.empresa_id == empresa_id, Partida.fecha >= fecha_inicio, Partida.fecha <= fecha_fin)
    ...
    def _construir_partida_out(
        self, partida: Partida, empresa_nombre: str, cuentas: dict[int, CuentaContable]
    ) -> PartidaOut:
