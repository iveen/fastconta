# app/services/periodos_service.py
...
async def validar_periodo_abierto_por_fecha(
    ...
    stmt = select(PeriodoFiscal).where(
        PeriodoFiscal.empresa_id == empresa_id, PeriodoFiscal.fecha_inicio <= fecha, PeriodoFiscal.fecha_fin >= fecha
    )
...
async def validar_periodo_abierto_por_id(
    db: AsyncSession,
    periodo_id: int,  # ✅ BIGINT (era UUID)
    empresa_id: int,  # ✅ BIGINT (era UUID)
) -> PeriodoFiscal:
    """
