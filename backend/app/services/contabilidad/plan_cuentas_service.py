...
async def procesar_importacion_excel(
    ...
    try:
        ...
        if errores:
            raise HTTPException(
                status_code=400, detail={"mensaje": "Errores de validación en el archivo", "errores": errores}
            )
...
async def exportar_plan_cuentas(empresa_id: int, db: AsyncSession, schema_name: str) -> io.BytesIO:  # ✅ BIGINT
    ...
    df = pd.DataFrame(
        rows, columns=["codigo", "nombre", "tipo", "naturaleza", "nivel", "acepta_tercero", "cuenta_padre_codigo"]
    )
