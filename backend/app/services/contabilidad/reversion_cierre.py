...
async def revertir_cierre_anual(
    ...
    try:
        ...
        if not periodo.cerrado:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="El período ya está abierto. No hay nada que revertir."
            )
        ...
        stmt_rev_existente = select(Partida).where(
            Partida.empresa_id == empresa_id, Partida.numero_poliza == patron_reversion
        )
        ...
        stmt_partidas_originales = (
            ...
            .where(Partida.empresa_id == empresa_id, Partida.numero_poliza.in_([patron_cierre, patron_ajuste]))
        )
        ...
        for partida_original in partidas_originales:
            for detalle in partida_original.detalles:
                ...
                detalles_invertidos.append(
                    {"cuenta_id": detalle.cuenta_id, "tipo_movimiento": tipo_invertido, "monto": detalle.monto}
                )
