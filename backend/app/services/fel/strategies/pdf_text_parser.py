...
async def parse_fel_texto(texto: str) -> Optional[Dict]:
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if "Nit Emisor" in line:
                ...
                for j in range(i - 1, max(0, i - 4), -1):
                    ...
                    if (
                        candidate
                        and "Factura" not in candidate
                        and "NÚMERO DE AUTORIZACIÓN" not in candidate
        ...
        es_exportacion = bool(
            re.search(r"Exportacion|ID\s+extranjero|COMPLEMENTO\s+EXPORTACION", texto_normalizado, re.IGNORECASE)
        )
        ...
        while i < len(lineas):
            ...
            if re.match(r"^\d+\s+(Servicio|Bien)", linea_actual, re.IGNORECASE):
                ...
                while j < len(lineas):
                    ...
                    if (
                        linea_sig
                        and not re.match(r"^\d+", linea_sig)
                        and not re.match(r"^(TOTALES|IVA|Impuestos)", linea_sig, re.IGNORECASE)
                        and len(linea_sig) < 50
                        and (linea_sig.isupper() or len(linea_sig.split()) == 1)
                    ):
        ...
        if es_exportacion:
            ...
            total_gravado_servicios = sum(item["total_linea"] for item in items if item["bien_o_servicio"] == "S")
            total_iva_servicios = sum(item["iva_linea"] for item in items if item["bien_o_servicio"] == "S")
            total_gravado_bienes = sum(item["total_linea"] for item in items if item["bien_o_servicio"] == "B")
            total_iva_bienes = sum(item["iva_linea"] for item in items if item["bien_o_servicio"] == "B")
