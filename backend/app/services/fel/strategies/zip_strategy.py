...
class ZipFelStrategy(FelIngestionStrategy):
    ...
    async def parse(self, content: FileContent, db) -> FelParsedResult:
        ...
        if not xml_files:
            return FelParsedResult(success=False, error="El ZIP no contiene archivos XML", source_format="zip")
        ...
        for xml_data in xml_files:
            try:
                ...
                if result.success:
                    ...
                    errores.append({"filename": xml_data["filename"], "error": result.error})
                    parsed_results.append({"filename": xml_data["filename"], "success": False, "error": result.error})

            except Exception as e:
                logger.error(f"Error procesando {xml_data['filename']}: {e}", exc_info=True)
                errores.append({"filename": xml_data["filename"], "error": str(e)})
                parsed_results.append({"filename": xml_data["filename"], "success": False, "error": str(e)})
