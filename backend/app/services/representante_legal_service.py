...
class RepresentanteLegalService:
    ...
    async def obtener_representantes_por_empresa(
        self, empresa_id: UUID, solo_activos: bool = True
        ...
        if solo_activos:
            query = query.where(RepresentanteLegal.es_activo.is_(True))
    ...
    async def obtener_representante_por_id(
        self, representante_id: UUID, empresa_id: UUID
    ...
    async def verificar_duplicado(
        ...
        representante_id_excluir: UUID | None = None,
    ) -> bool:
        """
        ...
        query = select(func.count(RepresentanteLegal.id)).where(
            and_(
                ...
                RepresentanteLegal.es_activo.is_(True),
            )
        )
    ...
    async def actualizar_representante(
        self, representante_id: UUID, empresa_id: UUID, data: dict
    ) -> RepresentanteLegal | None:
    ...
    async def eliminar_representante(
        ...
        """Elimina un representante legal (soft delete: es_activo = False)"""
        representante = await self.obtener_representante_por_id(representante_id, empresa_id)
        if representante is None:
            return False

        representante.es_activo = False
        await self.db.flush()
        return True