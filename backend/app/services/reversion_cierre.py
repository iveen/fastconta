...
async def revertir_cierre_anual(
    ...
    """
    Revierte un cierre contable creando una nueva partida de reversión (REV-Año).
    Esta partida contrarresta exactamente los movimientos del cierre (CIE) y ajuste (AJU),
    manteniendo la trazabilidad contable completa. ACID Compliant.
    """
    try:
        ...
        if not periodo.cerrado:
            raise HTTPException(
                ...
                detail="El período ya está abierto. No hay nada que revertir."
            )

        # 2. Regla de negocio: Verificar que no haya períodos posteriores cerrados
        ...
        if periodo_posterior_cerrado:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"No se puede revertir este cierre porque el período "
                       f"'{periodo_posterior_cerrado.nombre}' ya está cerrado. "
                       f"Debe revertir los cierres en orden cronológico inverso."
            )
        ...
        stmt_rev_existente = select(Partida).where(
            ...
            Partida.is_active.is_(True)
        )
        if (await db.execute(stmt_rev_existente)).scalar_one_or_none():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Ya existe una partida de reversión '{patron_reversion}' para este período."
            )
        ...
        stmt_partidas_originales = (
            select(Partida)
            .options(selectinload(Partida.detalles))  # ✅ Carga eager para evitar MissingGreenlet
            .where(
                Partida.empresa_id == empresa_id,
        ...
        if not partidas_originales:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"No se encontraron partidas de cierre (CIE/AJU) para el período '{periodo.nombre}'."
            )
        ...
        for partida_original in partidas_originales:
            for detalle in partida_original.detalles:
                ...
                # Invertir el tipo de movimiento: debe ↔ haber
        ...
        if not detalles_invertidos:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Las partidas de cierre originales no tienen detalles activos para revertir."
            )

        # 6. Crear la partida de reversión (REV)
        # ✅ Leemos el schema directamente de db.info, igual que en cierre_contable.py
        schema_name = db.info["current_user"]["schema"]
        await get_next_poliza(db, empresa_id, schema_name) # Generamos el consecutivo interno

        partida_rev = Partida(
            fecha=periodo.fecha_fin,
            descripcion=f"REVERSIÓN DE CIERRE CONTABLE - Período {periodo.nombre}. "
        ...
        for det in detalles_invertidos:
            db.add(DetallePartida(
                partida_id=partida_rev.id,
                cuenta_id=det['cuenta_id'],
                tipo_movimiento=det['tipo_movimiento'],
                monto=det['monto']
            ))

        # 7. Reabrir el período y hacer commit único (Atomicidad)
