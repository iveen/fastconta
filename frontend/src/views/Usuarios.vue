...
<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="max-w-6xl mx-auto space-y-6">
      ...
      <!-- Banner de estado -->
      <div v-if="statusMsg" :class="statusType === 'error' ? 'bg-red-100 border-red-400 text-red-700' : 'bg-green-100 border-green-400 text-green-700'" class="border px-4 py-3 rounded flex justify-between items-center">
        <span>{{ statusMsg }}</span>
      ...
      <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <!-- Formulario de Creación (simplificado para el ejemplo) -->
        <div v-if="showCreateForm" class="p-6 border-b bg-blue-50/50">
          <form @submit.prevent="handleCreateUser" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="newUser.full_name" type="text" required class="w-full border-gray-300 rounded-md p-2 border" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Correo Electrónico *</label>
              <input v-model="newUser.email" type="email" required class="w-full border-gray-300 rounded-md p-2 border" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Contraseña *</label>
              <input v-model="newUser.password" type="password" required minlength="8" class="w-full border-gray-300 rounded-md p-2 border" />
            </div>
            <div>
            ...
            <!-- El superadmin ya seleccionó el tenant arriba, lo inyectamos automáticamente -->
            <input v-if="authStore.isSuperAdmin" type="hidden" v-model="newUser.tenant_id" />

            <div class="md:col-span-2 flex justify-end gap-2 pt-2">
              <button type="button" @click="showCreateForm = false" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Cancelar</button>
              <button type="submit" :disabled="loadingCreate" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
                {{ loadingCreate ? 'Creando...' : 'Crear Usuario' }}
              </button>
            </div>
            <!-- Nota: Para crear usuario, asegúrate de que newUser.tenant_id se llene con selectedTenantId si es superadmin en el script -->
          </form>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="u in users" :key="u.id" @click="selectUser(u)" :class="['cursor-pointer hover:bg-blue-50 transition', selectedUser?.id === u.id ? 'bg-blue-50 border-l-4 border-blue-500' : '']">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ u.full_name }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ u.email }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">{{ formatRole(u.role) }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-blue-600 hover:text-blue-900 font-medium">Seleccionar</td>
            </tr>
            <tr v-if="users.length === 0"><td colspan="4" class="px-6 py-8 text-center text-gray-500">No hay usuarios registrados.</td></tr>
          </tbody>
        </table>
      </div>
        ...
        <div class="p-6">
          <div class="flex flex-col md:flex-row gap-4 items-end mb-6 bg-gray-50 p-4 rounded-lg">
            <div class="flex-1 w-full">
              ...
              <select v-model="assignData.empresa_id" class="w-full border-gray-300 rounded-md p-2 border">
                ...
                <option v-for="emp in availableEmpresas" :key="emp.id" :value="emp.id">{{ emp.nombre }} ({{ emp.nit }})</option>
              </select>
            </div>
            <button @click="handleAssignEmpresa" :disabled="!assignData.empresa_id || loadingAssign" class="w-full md:w-auto px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 disabled:opacity-50">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-if="userEmpresas.length === 0"><td colspan="3" class="px-6 py-8 text-center text-gray-500 italic">Este usuario no tiene empresas asignadas aún.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
...
const newUser = ref({ full_name: '', email: '', password: '', role: 'tenant_member', tenant_id: '' })
const assignData = ref({ empresa_id: '' })

const formatRole = (role) => {
  const map = { superadmin: 'Super Administrador', tenant_manager: 'Admin. Tenant', tenant_member: 'Miembro', tenant_client: 'Cliente' }
...
const selectUser = async (user) => {
  ...
  try {
    const res = await api.get(`/users/${user.id}/empresas`)
...
const handleCreateUser = async () => {
  ...
  try {
    ...
    if (authStore.isSuperAdmin) {
      payload.tenant_id = selectedTenantId.value // Forzar el tenant seleccionado
    }
    
    await api.post('/users/', payload)
    statusMsg.value = '✅ Usuario creado exitosamente.'
    statusType.value = 'success'
    newUser.value = { full_name: '', email: '', password: '', role: 'tenant_member', tenant_id: '' }
...
const handleAssignEmpresa = async () => {
  ...
  statusMsg.value = ''
  try {
    await api.post(`/users/${selectedUser.value.id}/empresas`, { empresa_id: assignData.value.empresa_id })
